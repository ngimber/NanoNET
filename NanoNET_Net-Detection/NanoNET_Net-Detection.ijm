...
linewidthUM          = 0.210;// Width of skeleton lines and profile traces (µm)
minlengthUM          = 0.225;// Minimum length for any measured profile (µm)
dnaChannel         = 1;// DNA image channel to process (1-based index)
...
Dialog.addDirectory("Analysis folder:", dir);
Dialog.addMessage("\n");
...
Dialog.addNumber("DNA channel (1-based):", dnaChannel);
...
print("Line width (pixels): " + linewidthUM);
selectWindow("Log");
...
for (i = 0; i < files.length; i++) {
    if (indexOf(files[i], extention) >= 0) {
        ...
        for (ch = 1; ch <= channels; ch++) {
            ...
            setBatchMode(false);
            // Loop through each ROI to extract profile
            for (n = 0; n < nRois; n++) {
                roiManager("Select", n);
                run("Area to Line"); roiManager("Add");
                profile = getProfile(); writeResulttabe(profile, n);
            }
            setBatchMode(true);
